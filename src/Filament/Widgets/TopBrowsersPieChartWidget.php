...
class TopBrowsersPieChartWidget extends ChartWidget
{
    public function getHeading(): string|Htmlable|null
    {
        return __('visitor-tracking-filament::widgets.top_browsers_pie_chart.heading');
    }
    ...
    protected function getData(): array
    {
        ...
        return app()->isLocal() ?
            ...
            :
            Cache::remember('top-browsers-pie-chart', 600, fn () => $this->generateChartData());
    }
    ...
    private function generateChartData(): array
    {
        ...
        $browserCounts = VisitorTracking::$visitorModel::where('is_bot', false)
            ...
            ->groupBy('browser')
            ->orderBy('count', 'desc')
            ->limit(8) // Limit to top 8 browsers for better pie chart readability
            ->pluck('count', 'browser')
            ->toArray();
        ...
        $countedBrowsers = array_sum($browserCounts);
        if ($totalBrowsers > $countedBrowsers) {
            $browserCounts['Others'] = $totalBrowsers - $countedBrowsers;
        }
        ...
        foreach ($browserCounts as $browser => $count) {
            ...
            $labels[] = ucfirst($browser).' ('.Number::forHumans($count, 2, 2, true).' - '.$percentage.'%)';
            $data[] = $count;
        }
        ...
        return [
            'datasets' => [
                [
                    'label' => __('visitor-tracking-filament::widgets.top_browsers_pie_chart.label'),
                    'data' => $data,
